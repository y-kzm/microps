...
struct nd6_cache {
    ...
    uint8_t state; 
    ip6_addr_t ip6addr;
    uint8_t lladdr[ETHER_ADDR_LEN]; /* link layer address */
    bool isrouter;
...
nd6_ns_output(struct ip6_iface *iface, const ip6_addr_t target)
{
    ...
    ns->nd_ns_sum = 0;
    ns->nd_ns_reserved = 0;
    IPV6_ADDR_COPY(&ns->target, &target, IPV6_ADDR_LEN);
...
nd6_na_output(uint8_t type, uint8_t code, uint32_t flags, const uint8_t *data, size_t len, ip6_addr_t src, ip6_addr_t dst, const ip6_addr_t target, const void *lladdr)
{
    ...
    */
    ...
    IPV6_ADDR_COPY(&na->target, &target, IPV6_ADDR_LEN);
    ...
    /* calculate the checksum */
    memset(&pseudo, 0, sizeof(struct ip6_pseudo_hdr));
    IPV6_ADDR_COPY(&pseudo.src, &src, IPV6_ADDR_LEN);
